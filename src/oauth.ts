...
export class GongfengOAuthController {
  ...
  /**
   * List repositories visible to the authorized current user.
   * @param config Current OAuth configuration.
   * @returns Repository identities suitable for source selection.
   */
  async repositories(config: GongfengOAuthConfig): Promise<MarketplaceRepository[]> {
    const visibleRepositories: MarketplaceRepository[] = []
    for (let page = 1; page <= 20; page += 1) {
      const url = new URL('/api/v3/projects', validateOAuthBaseUrl(config.baseUrl))
      url.searchParams.set('page', String(page))
      url.searchParams.set('per_page', '100')
      url.searchParams.set('order_by', 'last_activity_at')
      url.searchParams.set('sort', 'desc')
      const response = await this.fetch(config, url)
      const parsed = z.array(repositorySchema).parse(JSON.parse(await boundedText(response, API_RESPONSE_BYTES)))
      visibleRepositories.push(...parsed.map(entry => ({
        id: entry.id,
        path: entry.path_with_namespace,
        name: entry.name,
  ...
  private async fetch(config: GongfengOAuthConfig, url: URL): Promise<Response> {
    ...
    let response = await globalThis.fetch(url, { headers: { 'OAUTH-TOKEN': token } })
    if (response.status === 401) {
      token = await this.accessToken(config, true) ?? token
      response = await globalThis.fetch(url, { headers: { 'OAUTH-TOKEN': token } })
    }
    ...
    if (!response.ok) {
      const diagnostic = await boundedText(response, TOKEN_RESPONSE_BYTES)
      throw new Error(`marketplace Gongfeng API failed with ${String(response.status)}${diagnostic === '' ? '' : `: ${diagnostic}`}`)
    }
    return response
  }
