...
import { GongfengOAuthController, validateOAuthBaseUrl, type GongfengOAuthConfig } from './oauth.ts'
...
function validateSource(baseUrl: string, repository: string, ref: string): void {
  ...
  if (parsed.protocol !== 'https:') throw new Error('marketplace source requires an HTTPS base URL')
...
export class MarketplaceService extends Service {
   ...
   */
  async repositories(): Promise<MarketplaceRepository[]> {
  ...
  async add(packageName: string): Promise<MarketplaceMutationResult> {
    return await this.serializeMutation(async () => {
      ...
      await this.mutateProfile(
        ['add', artifact, '--save-exact', '--ignore-scripts'],
  ...
  private async repositoryFile(config: ResolvedConfig, path: string, maxBytes: number): Promise<Buffer> {
    ...
    if (credential === undefined) throw new Error(`marketplace credential "${config.tokenRef}" is not configured`)
