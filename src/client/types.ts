...
export interface MarketplaceState {
  ...
  readonly authMode: 'oauth' | 'token' | 'none'
...
export interface MarketplaceRemote {
  ...
  configure(request: { baseUrl: string; repository: string; ref: string; token?: string }): Promise<MarketplaceState>
  add(packageName: string): Promise<MutationResult>
  deletePackage(packageName: string): Promise<MutationResult>
  beginOAuth(request: { baseUrl: string; clientId: string }): Promise<OAuthStart>
